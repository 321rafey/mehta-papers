...
struct NOStep3View: View {
    ...
    @State private var selectedIsBoard = false
    @State private var lengthInput   = ""
    @State private var widthInput    = ""
    @State private var quantityInput = ""
    @State private var activeField   = "length"
    @State private var showPaperPicker = false

    // Auto-computed weight from dimensions
    // Board:  L × W × GSM / 15500 = kg per packet
    // Paper:  L × W × GSM / 3100  = kg per rim
    var autoWeight: Double {
        let l = Double(lengthInput) ?? 0
        let w = Double(widthInput)  ?? 0
        guard l > 0, w > 0, paperGSM > 0 else { return 0 }
        let divisor: Double = selectedIsBoard ? 15500 : 3100
        return (l * w * paperGSM) / divisor
    }

    var weightLabel: String { selectedIsBoard ? "kg / packet" : "kg / rim" }

    // Total PKR = rate/kg × weight × quantity
    var totalPKR: Double {
        let q = Double(quantityInput) ?? 0
        return paperRate * autoWeight * q
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPaperPicker) {
            PaperTypeSelectorView(
                ...
                selectedIsBoard:   $selectedIsBoard
            )
        }
    }
    ...
    var calculatorTabContent: some View {
        VStack(spacing: 12) {
            ...
            if showSendMenu {
                HStack(spacing: 8) {
                    ForEach(["Length", "Width", "Quantity"], id: \.self) { target in
                        Button {
                            let val = calcDisplay
                            switch target {
                            case "Length":   lengthInput   = val
                            case "Width":    widthInput    = val
        ...
        VStack(spacing: 0) {
            ScrollView(showsIndicators: false) {
                VStack(spacing: 12) {
                    ...
                    // Size: two fields — Length and Width
                    HStack(spacing: 10) {
                        SizeInputBox(label: "Length", value: $lengthInput, isActive: activeField == "length") { activeField = "length" }
                        SizeInputBox(label: "Width",  value: $widthInput,  isActive: activeField == "width")  { activeField = "width" }
                    }
                    .padding(.horizontal, 20)

                    // Auto-calculated weight — read-only display
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Weight (\(weightLabel))").font(.system(size: 11)).foregroundColor(Theme.subtitleText)
                            Text(autoWeight > 0 ? String(format: "%.3f", autoWeight) : "—")
                                .font(.system(size: 16, weight: .semibold))
                                .foregroundColor(autoWeight > 0 ? Color(hex: "#2A4A1A") : Theme.subtitleText)
                        }
                        Spacer()
                        Text(selectedIsBoard ? "L×W×GSM÷15500" : "L×W×GSM÷3100")
                            .font(.system(size: 10)).foregroundColor(Theme.subtitleText)
                    }
                    .padding(.horizontal, 14).padding(.vertical, 12)
                    .background(Color(hex: "#8FA87A").opacity(0.12))
                    .cornerRadius(10)
                    .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color(hex: "#8FA87A").opacity(0.3), lineWidth: 1))
                    .padding(.horizontal, 20)
    ...
    func applyKey(_ key: String, to field: String) {
        switch field {
        case "length":   updateField(&lengthInput,   key: key)
        case "width":    updateField(&widthInput,    key: key)
...
// MARK: - Size Input Box (compact, side-by-side for Length/Width)
struct SizeInputBox: View {
    let label: String
